...
DeepVoucher 是一个本地运行的自动录入凭证原型工程，面向“上传一组业务图片 -> 生成 1 张支持多借多贷的凭证 JSON”场景。项目结合多模态大模型、规则闸门、知识检索和人工确认闭环，目标是在保证安全约束的前提下，实现可追溯的智能财务录入。
...
- `scripts/bootstrap_knowledge.py`：将本地业务资料分发到项目目录并生成结构化知识产物
- `knowledge/`：知识库存储
- `data/`：运行产物
...
3. 当前已具备交付就绪检查、知识检索、学习记忆和可视化工作台
4. 当前仍在继续补通用规则库、更多真实样本回归和更强泛化能力
